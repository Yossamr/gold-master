...
export const convertArabicToEnglishNums = (str: string): string => {
  if (typeof str !== 'string') return str;
  const arabicNums = [/٠/g, /١/g, /٢/g, /٣/g, /٤/g, /٥/g, /٦/g, /٧/g, /٨/g, /٩/g];
  const persianNums = [/۰/g, /۱/g, /۲/g, /۳/g, /۴/g, /۵/g, /۶/g, /۷/g, /۸/g, /۹/g];
  let res = str;
  for (let i = 0; i < 10; i++) {
    res = res.replace(arabicNums[i], i.toString()).replace(persianNums[i], i.toString());
  }
  return res;
};

export const Input = forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement> & { label?: string }>(({ label, className, type, onChange, inputMode, ...props }, ref) => {
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (onChange) {
      const originalValue = e.target.value;
      const convertedValue = convertArabicToEnglishNums(originalValue);
      e.target.value = convertedValue;
      onChange(e);
    }
  };

  // Optimize for numeric keypad if type is "number" or tel
...
export const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: ReactNode }> = ({ isOpen, onClose, title, children }) => {
  ...
  return createPortal(
    <div 
      ...
      style={{ alignItems: 'flex-end', paddingBottom: '96px' }}
    >
      <div 
