...
import { StoreProfile, AppUser, UserRole, Transaction, TransactionType, GENERATE_ID } from '../types';
import { Save, Trash2, Plus, Store, Upload, Lock, FileText, Search, Printer, MessageCircle, ArrowUpRight, ArrowDownLeft, Calendar, User, Users, CheckCircle2, X, Receipt, Smartphone, Banknote, MapPin, Share, PlusSquare, Database, Key, Shield, AlertTriangle } from 'lucide-react';
...
const Settings = () => {
  ...
  // Database Connection settings states
  const [dbUrl, setDbUrl] = useState(localStorage.getItem('sys_db_url') || '');
  const [dbToken, setDbToken] = useState(localStorage.getItem('sys_db_token') || '');

  const [dbActivationKey, setDbActivationKey] = useState('');
  ...
  const handleUpdateDatabase = () => {
    ...
    if (!dbUrl || !dbToken) {
      setDbError('جميع الحقول مطلوبة لتشغيل النظام في وضع السحابة السيرفر');
      return;
    }

    if (!dbActivationKey) {
      setDbError('رمز تفعيل الأمان (2FA) مطلوب لإجراء هذا التغيير الحساس.');
      return;
    }
    ...
    try {
      ...
      if (!isValid) {
        setDbError('رمز تفعيل الأمان (2FA) غير صحيح أو منتهي الصلاحية.');
        return;
      }
    } catch (err: any) {
      setDbError('حدث خطأ أثناء التحقق من رمز الأمان: ' + (err.message || ''));
      return;
    }
    ...
    setDbActivationKey('');
    setDbSuccess('تم تحديث إعدادات قاعدة البيانات والاتصال بنجاح! سيتم إعادة تحميل التطبيق لتطبيق التغييرات.');
  ...
  const handleResetDatabase = async () => {
    setIsResetting(true);
    try {
      const success = await SettingsService.resetDatabase();
      if (success) {
        alert("تم تصفير النظام بنجاح.");
        window.location.reload();
      } else {
        alert("فشل تصفير النظام، يرجى التحقق من الاتصال والمحاولة مجدداً.");
      }
    } catch (e) {
        ...
        {activeTab === 'GENERAL' && (
            <form onSubmit={handleSaveProfile} className="animate-slide-up">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Reset Database Zone (Manager/Owner Only) */}
                    <div className="md:col-span-2">
                        <SectionHeader title="منطقة الخطر (Danger Zone)" />
                        <div className="bg-red-950/20 rounded-3xl overflow-hidden border border-red-500/20 shadow-xl p-6 space-y-4">
                            <div className="flex items-start gap-4">
                                <div className="p-3 bg-red-500/20 text-red-500 rounded-2xl shrink-0">
                                    <AlertTriangle size={24} />
                                </div>
                                <div>
                                    <h4 className="text-red-400 font-bold text-sm mb-1">تصفير النظام بالكامل</h4>
                                    <p className="text-[11px] text-gray-400 leading-relaxed">
                                        سيؤدي هذا الإجراء إلى حذف جميع البيانات المسجلة (المبيعات، المخزون، المصروفات، العملاء، الخزائن) بشكل نهائي واستعادتها لوضع البداية.
                                        لن يتم حذف <span className="font-bold text-white">المستخدمين</span> أو <span className="font-bold text-white">إعدادات المحل</span> أو <span className="font-bold text-white">أسعار الذهب</span>.
                                    </p>
                                </div>
                            </div>
                            
                            {!isManagerOrOwner ? (
                                <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl text-xs text-center font-bold">
                                    هذه الميزة متاحة فقط للمدير أو المالك.
                                </div>
                            ) : !showResetConfirm ? (
                                <button
                                    type="button"
                                    onClick={() => setShowResetConfirm(true)}
                                    className="w-full bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 rounded-xl py-3 text-xs font-bold transition-all"
                                >
                                    طلب تصفير النظام
                                </button>
                            ) : (
                                <div className="bg-red-500/10 p-4 rounded-xl border border-red-500/20 space-y-3">
                                    <p className="text-red-400 text-xs font-bold text-center">هل أنت متأكد من رغبتك في مسح البيانات؟ لا يمكن التراجع عن هذا الإجراء.</p>
                                    <div className="flex gap-2">
                                        <button
                                            type="button"
                                            onClick={() => setShowResetConfirm(false)}
                                            className="flex-1 bg-white/5 hover:bg-white/10 text-white rounded-lg py-2.5 text-xs font-bold transition-all"
                                        >
                                            إلغاء
                                        </button>
                                        <button
                                            ...
                                            className="flex-1 bg-red-500 hover:bg-red-600 text-white rounded-lg py-2.5 text-xs font-black transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                                        >
                                            {isResetting ? 'جاري الحذف...' : 'نعم، قم بتصفير النظام الآن'}
                                        </button>
                                    </div>
                                </div>
                            )}
                    ...
                    <div>
                        ...
                        <div className="bg-[#1e293b] rounded-3xl overflow-hidden border border-white/5 shadow-xl">
                            ...
                            <SettingRow>
                                <div className="flex-1">
                                    <label className="block text-[10px] text-amber-500 font-bold mb-1">حساب إنستا باي (InstaPay) الموحد</label>
                                    <input className="bg-transparent w-full text-white font-medium outline-none placeholder-gray-600 font-mono" value={profile.instapayAccount || ''} onChange={e => setProfile({...profile, instapayAccount: e.target.value})} placeholder="username@instapay" />
                                </div>
                            </SettingRow>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2">
                        <SectionHeader title="إعدادات ربط قاعدة البيانات والسيرفر (Turso Cloud / Local)" />
                        <div className="bg-[#1e293b] rounded-3xl overflow-hidden border border-white/5 shadow-xl p-6 space-y-6">
                                يمكنك تغيير رابط الـ API ورمز الاتصال (Token) بقاعدة بيانات Turso السحابية من هنا لتغيير السيرفر.

                            {dbError && (
                                <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl text-xs flex items-center gap-3 font-bold animate-scale-in">
                            ...
                            {dbSuccess && (
                                <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-4 rounded-2xl text-xs flex items-center gap-3 font-bold animate-scale-in">
                            ...
                            <div className="space-y-4">
                                    <div className="space-y-4 animate-slide-up">
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-gray-400 flex items-center gap-2">
                                                ...
                                                <span>رابط قاعدة البيانات (Database URL)</span>
                                            </label>
                                            <input 
                                                className="w-full bg-[#0a0e1a]/60 border border-white/5 text-white px-4 py-3.5 rounded-xl focus:border-gold-500/50 outline-none font-mono text-xs transition-all placeholder-gray-700"
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <textarea 
                                                className="w-full bg-[#0a0e1a]/60 border border-white/5 text-white px-4 py-3 rounded-xl focus:border-gold-500/50 outline-none font-mono text-[10px] h-20 resize-none transition-all placeholder-gray-700 p-3 leading-relaxed"
                                ...
                                {/* Validation Code */}
                                <div className="bg-[#0f172a] p-4 rounded-2xl border border-white/5 space-y-3">
                                    <label className="text-xs font-bold text-gold-400 flex items-center gap-2">
                                        <Lock size={14} />
                                        <span>رمز تأكيد الأمان للمسؤول (2FA Token)</span>
                                    </label>
                                    <div className="flex gap-3">
                                        <input 
                                            ...
                                            inputMode="numeric"
                                            placeholder="••••••" 
                                            value={dbActivationKey} 
                                            onChange={e => setDbActivationKey(e.target.value)} 
                                            className="bg-black/60 border border-gold-900/60 focus:border-gold-500/80 rounded-xl px-4 py-2 text-center font-bold text-lg text-gold-400 outline-none tracking-[0.3em] w-36 placeholder-gray-800"
                                            maxLength={6}
                                        />
                                        <button 
                                            type="button"
                                            onClick={handleUpdateDatabase}
                                            className="flex-1 bg-gradient-to-r from-gold-600 to-gold-500 hover:from-gold-500 hover:to-gold-400 text-black font-black rounded-xl text-xs transition-all active:scale-95 flex items-center justify-center gap-1.5"
                                        >
                                            <Lock size={14} />
                                            <span>تحديث وحفظ الاتصال</span>
                                        </button>
                                    </div>
                                    <p className="text-[10px] text-gray-500">ملاحظة: يتطلب تغيير إعدادات الربط رمز أمان 2FA فعال.</p>
                                </div>
                            </div>
                        </div>
                    </div>
