...
const Safes = () => {
  ...
  const [allSafesForTransfer, setAllSafesForTransfer] = useState<Safe[]>([]);
  ...
  const refresh = () => {
      ...
      const allActiveSafes = SafeService.getAll();
      const shopSafes = allActiveSafes.filter(s => (s.shop || 'المحل الأساسي') === currentShop);
      setSafes(shopSafes);

      // Get ALL raw safes across both shops for cross-shop transfer
      const raw = SafeService._getAllRaw();
      setAllSafesForTransfer(raw);

      // Auto-set transfer defaults if empty using the raw list
      if (raw.length >= 2 && (!transferData.from || !transferData.to)) {
          const currentShopSafes = raw.filter(s => (s.shop || 'المحل الأساسي') === currentShop);
          const otherShopSafes = raw.filter(s => (s.shop || 'المحل الأساسي') !== currentShop);
          
          setTransferData({
              from: currentShopSafes[0]?.id || raw[0].id,
              to: otherShopSafes[0]?.id || raw[1].id,
              amount: ''
          });
      }
  ...
  const handleTransfer = (e: React.FormEvent) => {
    ...
    const sourceSafe = allSafesForTransfer.find(s => s.id === transferData.from);
    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-4 pb-32 space-y-6 safe-pt animate-fade-in">
      ...
      <div className="flex p-1 bg-[#1e293b]/50 backdrop-blur rounded-2xl border border-white/5">
          ...
          <button 
             ...
             disabled={allSafesForTransfer.length < 2}
             className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${allSafesForTransfer.length < 2 ? 'opacity-40 cursor-not-allowed' : ''} ${activeTab === 'TRANSFER' ? 'bg-[#f4c025] text-black shadow-lg shadow-amber-500/10' : 'text-gray-400 hover:text-white'}`}
          >
      ...
      {activeTab === 'TRANSFER' && allSafesForTransfer.length >= 2 && (
              ...
              <form onSubmit={handleTransfer} className="space-y-4">
                 <div className="relative bg-[#0f172a] rounded-3xl p-2 border border-white/5">
                     ...
                     <div className="p-4 text-right">
                          ...
                          >
                             {allSafesForTransfer.map(s => <option key={s.id} value={s.id} className="bg-[#1e293b] text-gray-300">{s.name} - {s.shop || 'المحل الأساسي'} ({s.balance.toLocaleString()} ج.م)</option>)}
                          </select>
                     </div>
                    ...
                    {selectedTx.fromSafeId && (
                        <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2.5">
                            ...
                            <span className="text-rose-400 font-bold">
                                {(() => {
                                    const s = SafeService._getAllRaw().find(x => x.id === selectedTx.fromSafeId);
                                    return s ? `${s.name} (${s.shop || 'المحل الأساسي'})` : selectedTx.fromSafeId;
                                })()}
                            </span>
                        </div>
                    )}
                    ...
                    {selectedTx.toSafeId && (
                        <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2.5">
                            ...
                            <span className="text-emerald-400 font-bold">
                                {(() => {
                                    const s = SafeService._getAllRaw().find(x => x.id === selectedTx.toSafeId);
                                    return s ? `${s.name} (${s.shop || 'المحل الأساسي'})` : selectedTx.toSafeId;
                                })()}
                            </span>
                        </div>
                    )}
