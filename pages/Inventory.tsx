...
import { Search, Printer, Box, ArrowRightLeft, Plus, QrCode, AlertCircle, Eye } from 'lucide-react';
import { Modal, Input, Select, convertArabicToEnglishNums } from '../components/UI';
...
const Inventory = () => {
  ...
  const [scannedExistingItem, setScannedExistingItem] = useState<Item | null>(null);
  ...
  const handleBarcodeDetected = (code: string) => {
      ...
      const foundItem = allItems.find(i => (i.barcode || '').trim().toLowerCase() === codeClean);
      
      if (foundItem) {
          setScannedExistingItem(foundItem);
      } else {
          setQuickAddForm({
              name: '',
              weight: '',
    ...
    <div className="px-4 py-4 space-y-4 font-sans" dir="rtl">
      ...
      <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-gold-500 hover:bg-gold-600 text-black text-xs font-black rounded-xl shadow-lg active:scale-95 transition-all"
              >
                  <QrCode size={15} />
                  <span>إضافة / سكان صنف</span>
              </button>
              <button className="p-2.5 bg-dark-card rounded-xl text-gray-400 hover:text-white border border-dark-border active:scale-95 transition-all">
                  <Printer size={18} />
              </button>
          </div>
      </div>
      ...
      <div className="relative">
          ...
          <input 
            ...
            value={filterText}
            onChange={e => setFilterText(convertArabicToEnglishNums(e.target.value))}
          />
      </div>
      ...
      {/* Existing Item Scanned Modal */}
      <Modal
        isOpen={!!scannedExistingItem}
        onClose={() => setScannedExistingItem(null)}
        title="الصنف مسجل بالفعل"
      >
        {scannedExistingItem && (
          <div className="space-y-4 text-right" dir="rtl">
            <div className="flex items-center gap-2 text-amber-500 font-bold justify-center bg-amber-500/10 p-3 rounded-xl border border-amber-500/20">
                <AlertCircle size={20} />
                <span>هذا الباركود معرف مسبقاً في المخزن</span>
            </div>
            ...
            <div className="bg-dark-bg p-4 rounded-xl border border-dark-border space-y-3">
              <div className="flex justify-between border-b border-dark-border pb-2">
                <span className="text-gray-400 text-xs">اسم الصنف:</span>
                <span className="text-white font-bold">{scannedExistingItem.name}</span>
              </div>
              <div className="flex justify-between border-b border-dark-border pb-2">
                <span className="text-gray-400 text-xs">رقم الباركود:</span>
                <span className="text-white font-mono font-bold bg-black/40 px-2 py-0.5 rounded">{scannedExistingItem.barcode}</span>
              </div>
              <div className="flex justify-between border-b border-dark-border pb-2">
                <span className="text-gray-400 text-xs">العيار:</span>
                <span className="text-gold-500 font-bold">عيار {scannedExistingItem.karat}</span>
              </div>
              <div className="flex justify-between border-b border-dark-border pb-2">
                <span className="text-gray-400 text-xs">الوزن الحالي:</span>
                <span className="text-white font-bold font-mono">{scannedExistingItem.weight.toFixed(2)} جرام</span>
              </div>
              <div className="flex justify-between border-b border-dark-border pb-2">
                <span className="text-gray-400 text-xs">الفرع / المحل:</span>
                <span className="text-white font-bold">{scannedExistingItem.shop || 'المحل الأساسي'}</span>
              </div>
              <div className="flex justify-between border-b border-dark-border pb-2">
                <span className="text-gray-400 text-xs">الحالة:</span>
                <span className={`font-bold ${scannedExistingItem.status === ItemStatus.IN_STORE ? 'text-emerald-400' : 'text-gray-400'}`}>
                  {scannedExistingItem.status === ItemStatus.IN_STORE ? 'متوفر بالمحل' : 'مباع / غير متوفر'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400 text-xs">المصنعية للجرام:</span>
                <span className="text-white font-bold font-mono">{scannedExistingItem.workmanship || 0} ج.م</span>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <button 
                type="button" 
                onClick={() => {
                  setSelectedItemForTransfer(scannedExistingItem);
                  setScannedExistingItem(null);
                }}
                className="flex-1 py-3 bg-gold-500 hover:bg-gold-600 text-black font-black rounded-xl active:scale-95 transition-transform flex items-center justify-center gap-1.5 text-sm"
              >
              ...
              <button 
                type="button" 
                onClick={() => setScannedExistingItem(null)}
                className="flex-1 py-3 bg-gray-800 hover:bg-gray-700 text-white font-bold rounded-xl active:scale-95 transition-transform text-sm"
              >
                إغلاق
              </button>
            </div>
