...
const Reps = React.lazy(() => import('./pages/Reps'));
const Settings = React.lazy(() => import('./pages/Settings'));
const Customers = React.lazy(() => import('./pages/Customers'));
const Safes = React.lazy(() => import('./pages/Safes'));
const Ghawayesh = React.lazy(() => import('./pages/Ghawayesh'));
const Bullions = React.lazy(() => import('./pages/Bullions'));
...
const TechSetup = React.lazy(() => import('./pages/TechSetup'));
...
const MobileHeader = ({ title, onSync, isSyncing, onSettings, selectedShop, onSwitchShop }: { title: string, onSync: () => void, isSyncing: boolean, onSettings: () => void, selectedShop: string | null, onSwitchShop: () => void }) => {
  ...
  return (
    <div className="bg-[#0f172a] px-4 pt-safe-top pb-3 flex justify-between items-center border-b border-white/5 safe-pt max-w-7xl mx-auto w-full">
...
const PageSuspenseFallback = () => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in">
    <div className="relative mb-6">
      <div className="w-16 h-16 rounded-full flex items-center justify-center animate-spinSlow"
           style={{ background: 'conic-gradient(from 0deg, #f59e0b 0%, rgba(245, 158, 11, 0.1) 50%, transparent 100%)', padding: '2px' }}>
        <div className="w-full h-full rounded-full bg-dark-bg flex items-center justify-center">
           <AppIcon size={24} />
        </div>
      </div>
    </div>
    <div className="text-amber-500/70 text-xs font-bold animate-pulse tracking-widest">جاري التحميل...</div>
  </div>
);
...
const App = () => {
  ...
  const initApp = useCallback(async () => {
     setInitError(null);
     const wasSetup = localStorage.getItem('gold_app_is_setup') === 'true';

     try {
       // If the app was already setup, we can use a shorter timeout (e.g. 4 seconds) to try to connect to DB,
       // but if it fails or times out, we just proceed silently so the user is never blocked.
       // If the app was NOT setup, we use a 25-second timeout since they need to establish the connection first.
       const timeoutDuration = wasSetup ? 4000 : 25000;
       ...
       const initPromise = async () => {
         ...
         return await SettingsService.isAppInitialized();
       };
       ...
       try {
         ...
         if (setup) {
           localStorage.setItem('gold_app_is_setup', 'true');
         }
       } catch (err) {
         console.warn("DB Connection failed or timed out:", err);
         if (wasSetup) {
           // Gracefully fallback to setup = true if we were already setup before
           setup = true;
         } else {
           // Rethrow if we weren't setup, as we must establish the database on first run
           throw err;
         }
       }
       ...
       // Ensure sync runs in background on app startup if online
       if (navigator.onLine) {
           SyncService.sync().catch(console.error);
       }
     } catch (err: any) {
       ...
       if (err.message === "Timeout") {
         setInitError("انتهت مهلة الاتصال بخادم قاعدة البيانات. يرجى التحقق من جودة الإنترنت أو صحة الرابط والرمز.");
       } else if (err.message === "DB_INIT_FAILED") {
         setInitError("فشل الاتصال بقاعدة البيانات. تأكد من أن الرابط والرمز المدخلين صحيحين وأن خادم قاعدة البيانات يعمل.");
       } else {
         setInitError("حدث خطأ أثناء تهيئة النظام: " + (err.message || "خطأ مجهول"));
       }
     }
  }, []);
      ...
      if (isSetup === null) return (
        <div className="flex flex-col h-screen items-center justify-center bg-gradient-to-br from-[#070b19] via-[#0b1329] to-[#040812] relative overflow-hidden" dir="rtl">
          ...
          <div className="flex flex-col items-center bg-[#111c35]/30 backdrop-blur-2xl border border-white/5 rounded-[2.5rem] p-10 max-w-sm w-[90%] mx-auto shadow-[0_30px_100px_rgba(0,0,0,0.8)] relative z-10">
            ...
            <div className="relative mb-8">
              <div 
                ...
                style={{
                  ...
                  animation: initError ? 'none' : 'spinSlow 2.5s linear infinite',
                  padding: '2.5px',
                  borderRadius: '50%'
                }}
              >
                <div className="w-full h-full rounded-full bg-[#0b1329] flex items-center justify-center">
                  {/* Inside Logo Center */}
                  <div className="w-24 h-24 rounded-full bg-gradient-to-br from-amber-500/20 to-amber-900/10 border border-amber-500/20 flex items-center justify-center shadow-[inset_0_0_20px_rgba(245,158,11,0.1)]">
                    <svg width="48" height="48" viewBox="0 0 512 512" fill="none" className="drop-shadow-[0_0_15px_rgba(245,158,11,0.4)] animate-gold-glow">
                      <defs>
            ...
            {/* Typography */}
            <h1 className="text-3xl font-extrabold text-shimmer mb-1.5 tracking-wide text-center">Gold Master Egypt</h1>
            ...
            {initError ? (
              <div className="w-full space-y-4 animate-scale-in">
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl text-xs text-center leading-relaxed">
                  {initError}
                </div>
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <button 
                      onClick={() => {
                        localStorage.removeItem('sys_db_url');
                        localStorage.removeItem('sys_db_token');
                        localStorage.removeItem('sys_use_local_only');
                        setIsTechSetup(false);
                        setInitError(null);
                      }}
                      className="flex-1 bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-white rounded-xl py-2.5 text-xs font-bold transition-all active:scale-95"
                    >
                      تغيير الإعدادات
                    </button>
                    <button 
                      onClick={initApp}
                      className="flex-1 bg-gradient-to-r from-gold-600 to-gold-500 hover:from-gold-500 hover:to-gold-400 text-black rounded-xl py-2.5 text-xs font-black transition-all active:scale-95"
                    >
                      إعادة المحاولة
                    </button>
                  </div>
                  ...
                  <button 
                    onClick={() => {
                      localStorage.setItem('sys_use_local_only', 'true');
                      setInitError(null);
                      initApp();
                    }}
                    className="w-full bg-amber-500/10 border border-amber-500/20 hover:bg-amber-500/20 text-amber-400 rounded-xl py-2.5 text-xs font-bold transition-all active:scale-95"
                  >
                    التشغيل في الوضع المحلي (بدون إنترنت/سيرفر)
                  </button>
                </div>
              </div>
            ) : (
