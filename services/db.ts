...
export const DbService = {
  ...
  init: async () => {
    ...
    try {
      ...
      console.warn("DB Init Error (likely offline or missing config):", error);
      return false;
    }
  },
  ...
  login: async (email: string, pin: string): Promise<AppUser | null> => {
    ...
    try {
      ...
      if (rs.rows.length > 0) {
        const u = rs.rows[0];
        const isValid = await verifyPin(pin, u.pin as string);
        if (isValid) {
          return {
            id: u.id as string,
  ...
  clearAllDataExceptStore: async (keysToKeep: string[]): Promise<boolean> => {

    try {
      const db = getDb();
      const placeholders = keysToKeep.map(() => '?').join(',');
      await db.execute({
        sql: `DELETE FROM store_data WHERE key NOT IN (${placeholders})`,
        args: keysToKeep
      });
      return true;
    } catch (e) {
