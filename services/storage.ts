...
import { Item, Transaction, Rep, RepLog, ItemStatus, Karat, TransactionType, GoldMarketPrice, StoreProfile, AppUser, UserRole, ThemeMode, Customer, Safe, SafeTransaction, PaymentMethod, GhawayeshEntry, Expense, TIMESTAMP, GENERATE_ID, ScrapGoldLog, BullionItem, BullionLog } from '../types';
...
const KEYS = {
  ...
  BULLIONS: 'gold_bullions',
  BULLION_LOGS: 'gold_bullion_logs',
...
export const setStorageItem = set;
  ...
  sync: async () => {
    ...
    try {
        ...
        const keysToSync = [
            ...
            KEYS.BULLIONS, KEYS.BULLION_LOGS
        ];
...
export const SafeService = {
  ...
  getAll: (): Safe[] => {
    ...
    // Sync InstaPay balances across both shops to ensure they act as one unified account
    const instapay1 = safes.find(s => s.id === 'SAFE_INSTAPAY_1');
    const instapay2 = safes.find(s => s.id === 'SAFE_INSTAPAY_2');
    if (instapay1 && instapay2 && instapay1.balance !== instapay2.balance) {
      const unifiedBalance = Math.max(instapay1.balance, instapay2.balance);
      instapay1.balance = unifiedBalance;
      instapay2.balance = unifiedBalance;
      set(KEYS.SAFES, safes, false);
    }
    ...
    safes = safes.map(s => {
      if (!s.shop) {
        ...
        if (s.id === 'SAFE_1' || s.id === 'SAFE_INSTAPAY' || s.id === 'SAFE_INSTAPAY_1') {
          s.shop = 'المحل الأساسي'; migrated = true;
        } else if (s.id === 'SAFE_2' || s.id === 'SAFE_INSTAPAY_2') {
          s.shop = 'المحل الثاني'; migrated = true;
        }
      }
      if (s.id === 'SAFE_1' && s.name !== 'خزنة المحل الأساسي (Cash)') {
        s.name = 'خزنة المحل الأساسي (Cash)'; migrated = true;
      }
      if (s.id === 'SAFE_2' && s.name !== 'خزنة المحل الثاني (Cash)') {
        s.name = 'خزنة المحل الثاني (Cash)'; migrated = true;
      }
      // Ensure shared name for InstaPay to indicate they are unified
      if ((s.id === 'SAFE_INSTAPAY_1' || s.id === 'SAFE_INSTAPAY_2') && s.name !== 'حساب انستا باي المشترك') {
        s.name = 'حساب انستا باي المشترك'; migrated = true;
      }
      return s;
    });
  ...
  _getAllRaw: (): Safe[] => {
    let safes = get<Safe>(KEYS.SAFES);
    // Keep InstaPay synced in raw list too
    const instapay1 = safes.find(s => s.id === 'SAFE_INSTAPAY_1');
    const instapay2 = safes.find(s => s.id === 'SAFE_INSTAPAY_2');
    if (instapay1 && instapay2 && instapay1.balance !== instapay2.balance) {
      const unifiedBalance = Math.max(instapay1.balance, instapay2.balance);
  ...
  initDefaults: () => {
      ...
      const safes = [
        ...
        { id: 'SAFE_INSTAPAY_1',  name: 'حساب انستا باي المشترك',           shop: 'المحل الأساسي', balance: 0, updatedAt: TIMESTAMP() },
        { id: 'SAFE_2',           name: 'خزنة المحل الثاني (Cash)',         shop: 'المحل الثاني',  balance: 0, updatedAt: TIMESTAMP() },
        { id: 'SAFE_INSTAPAY_2',  name: 'حساب انستا باي المشترك',           shop: 'المحل الثاني',  balance: 0, updatedAt: TIMESTAMP() },
      ];
  ...
  updateBalance: (safeId: string, amount: number) => {
    ...
    const isInstapay = safeId === 'SAFE_INSTAPAY_1' || safeId === 'SAFE_INSTAPAY_2';
    
    let updated = false;
    safes.forEach(s => {
      if (s.id === safeId || (isInstapay && (s.id === 'SAFE_INSTAPAY_1' || s.id === 'SAFE_INSTAPAY_2'))) {
        s.balance += amount;
  ...
  transfer: (fromId: string, toId: string, amount: number, user: string) => {
    ...
    // Helper to get safe name with shop
    const allRaw = SafeService._getAllRaw();
    const fromSafe = allRaw.find(s => s.id === fromId);
    const toSafe = allRaw.find(s => s.id === toId);
    const fromName = fromSafe ? `${fromSafe.name} (${fromSafe.shop || 'المحل الأساسي'})` : fromId;
    const toName = toSafe ? `${toSafe.name} (${toSafe.shop || 'المحل الأساسي'})` : toId;
    ...
    const tx: SafeTransaction = {
      ...
      amount,
      type: 'TRANSFER',
      description: `تحويل رصيد من ${fromName} إلى ${toName}`,
      date: Date.now(),
      byUser: user,
      source: CURRENT_SOURCE, // Inject Source
      updatedAt: TIMESTAMP()
...
export const InventoryService = {
  ...
  getByBarcode: (barcode: string): Item | undefined => {
    ...
    return items.find(i => (i.barcode || '').trim().toLowerCase() === cleanBarcode && (i.shop || 'المحل الأساسي') === currentShop);
  },
...
export const TransactionService = {
  ...
  add: (tx: Transaction) => {
    ...
    if (tx.paymentMethod === PaymentMethod.SPLIT) {
      const cashSafeId = activeShop === 'المحل الثاني' ? 'SAFE_2' : 'SAFE_1';
      const instapaySafeId = activeShop === 'المحل الثاني' ? 'SAFE_INSTAPAY_2' : 'SAFE_INSTAPAY_1';
      const cashAmt = tx.cashAmount || 0;
      const instapayAmt = tx.instapayAmount || 0;
      if (tx.type === TransactionType.SALE || tx.type === TransactionType.DEBT_PAYMENT) {
        if (cashAmt > 0) SafeService.updateBalance(cashSafeId, cashAmt);
        if (instapayAmt > 0) SafeService.updateBalance(instapaySafeId, instapayAmt);
      } else {
        if (cashAmt > 0) SafeService.updateBalance(cashSafeId, -cashAmt);
        if (instapayAmt > 0) SafeService.updateBalance(instapaySafeId, -instapayAmt);
      }
...
export const GoldPriceService = {
  ...
  setPrice: (base21: number, manualOverride: boolean = true) => {
    const current = GoldPriceService.getStoredPrice();
    const price: GoldMarketPrice = { ...current, base21, lastUpdated: Date.now(), manualOverride };
  ...
  fetchLivePrice: async (): Promise<{base21: number, ounce: number}> => {
    ...
    try {
      ...
      // Helper to fetch with proxies
      const fetchHtmlWithProxies = async (url: string): Promise<string | null> => {
        // Proxy 1: corsproxy.io
        try {
          const res = await fetch(`https://corsproxy.io/?url=${encodeURIComponent(url)}&ts=${ts}`).then(r => r.text());
          if (res && res.length > 200) return res;
        } catch (e) {
          console.warn(`corsproxy.io failed for ${url}`, e);
        }
        
        // Proxy 2: allorigins
        try {
          const res = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}&timestamp=${ts}`).then(r => r.json());
          if (res && res.contents) return res.contents;
        } catch (e) {
          console.warn(`allorigins failed for ${url}`, e);
        }
      ...
      // Source 1: Isagha (Primary)
      try {
        const isaghaHtml = await fetchHtmlWithProxies('https://isagha.com/');
        if (isaghaHtml) {
          const match21 = isaghaHtml.match(/عيار\s*21[^0-9]*([\d,]{4,5})/);
        ...
        console.error("Isagha parsing failed", e);
      }

      // Source 2: Edahabapp (Fallback 1)
      if (newBase21 === 0) {
        try {
          const edahabHtml = await fetchHtmlWithProxies('https://edahabapp.com/');
          if (edahabHtml) {
            const match21 = edahabHtml.match(/عيار\s*21[^0-9]*([\d,]{4,5})/);
            if (match21 && match21[1]) {
              const val = parseInt(match21[1].replace(/,/g, ''), 10);
                            if (val > 2500 && val < 6000) {
                // SECURITY TODO: We should rely on a backend to provide verified prices.
          ...
          console.error("Edahabapp parsing failed", e);
        }
      }

      // Source 3: Dahabmasr (Fallback 2)
      if (newBase21 === 0) {
        try {
          const dahabmasrHtml = await fetchHtmlWithProxies('https://dahabmasr.com/');
          if (dahabmasrHtml) {
            const match21 = dahabmasrHtml.match(/21[^0-9]*([\d,]{4,5})/);
            if (match21 && match21[1]) {
              const val = parseInt(match21[1].replace(/,/g, ''), 10);
                            if (val > 2500 && val < 6000) {
                // SECURITY TODO: We should rely on a backend to provide verified prices.
          ...
          console.error("Dahabmasr parsing failed", e);
        }
      }
      ...
      // Source A: Direct Call to Gold-API
      try {
        ...
        console.warn("Gold-API fetch failed, trying fallback sources...", e);
      }
      ...
      if (newOunce === 0) {
        try {
          ...
          console.warn("Investing.com Arabic fetch failed, trying english source...", e);
        }
      }
      ...
      if (newOunce === 0) {
        try {
          ...
          console.warn("Investing.com English fetch failed, trying coingecko...", e);
        }
      }
      ...
      if (newOunce === 0) {
        try {
          ...
          console.warn("CoinGecko PAXG fetch failed", e);
        }
      }
      ...
      // Only record and dispatch base21 update if manualOverride is false
      if ((final21 !== current.base21 && !current.manualOverride) || finalOunce !== current.ouncePriceUsd) {
          const result: GoldMarketPrice = { 
            base21: current.manualOverride ? current.base21 : final21, 
            ouncePriceUsd: finalOunce, 
      ...
      return { base21: current.manualOverride ? current.base21 : final21, ounce: finalOunce };
    } catch (error) { 
      return { base21: current.base21, ounce: current.ouncePriceUsd }; 
    }
  },
...
export const SettingsService = {
  resetDatabase: async (): Promise<boolean> => {
    const keysToKeep = [
      KEYS.STORE_PROFILE,
      KEYS.USERS,
      KEYS.GOLD_PRICE_HISTORY,
      KEYS.GOLD_PRICE,
      KEYS.IS_SETUP,
      KEYS.THEME,
      KEYS.SYNC_TIMESTAMPS,
      'printed_barcodes_log',
      'sys_db_url',
      'sys_db_token',
      'sys_use_local_only'
    ];

    // Clear local storage for keys NOT in keysToKeep (by gathering keys first to avoid index-shifting)
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('gold_') && !keysToKeep.includes(key)) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));

    // Call DbService to clear remote tables except keys to keep
    const success = await DbService.clearAllDataExceptStore(keysToKeep);
    if (success) {
      // Re-init defaults for things like Safes that need to exist
  ...
  initializeSystem: async (storeName: string, slogan: string, ownerName: string, pin: string, email: string) => {
      ...
      const profile: StoreProfile = { name: storeName, slogan: slogan || '', address1: '', address2: '', phonePrimary: '', phoneSecondary: '', contacts: [], instapayAccount: '' };
      set(KEYS.STORE_PROFILE, profile);
      
      // 2. Register Owner in DB
      const owner: AppUser = { id: 'OWNER_1', name: ownerName, email, role: UserRole.OWNER, pin: await hashPin(pin), active: true };
  ...
  getStoreProfile: (): StoreProfile => {
    ...
    if (data) {
      ...
      return {
        ...parsed,
        instapayAccount: parsed.instapayAccount || parsed.instapayMainShop || parsed.instapaySecondShop || ''
      };
    }
    return { name: "", slogan: "", address1: "", address2: "", phonePrimary: "", phoneSecondary: "", contacts: [], instapayAccount: '' };
  },
...
export const AuthService = {
  login: async (email: string, pin: string) => {
     let user = await DbService.login(email, pin);
     
     // Offline fallback: check local users if remote login returned null
     if (!user) {
        const localUsersJson = localStorage.getItem('users');
        if (localUsersJson) {
           try {
              const localUsers = JSON.parse(localUsersJson) as AppUser[];
              let found = null;
              ...
              if (found) {
                 user = found;
                 console.log("Logged in via local authentication fallback.");
              }
           } catch (err) {
              console.error("Local login parsing failed:", err);
           }
        }
     }
...
window.onload=function(){
  ...
  printBarcodeTickets: async (tickets: (string | Item)[]) => {
     const printWindow = window.open('', '_blank', 'width=950,height=600');
     if (!printWindow) return;
     
     const store = SettingsService.getStoreProfile();
     const storePhone = store.phonePrimary || '01018866315';
     ...
     const blocksHtmlArray = await Promise.all(pairs.map(async ([t1, t2]) => {
         ...
         const renderWingContent = async (data: {barcode: string} | null) => {
             if (!data) return '';
             const qrData = await QRCode.toDataURL(data.barcode, { margin: 0, width: 120, color: { dark: '#000000', light: '#ffffff' } });
             return `
                 <!-- Top Half (Barcode drawn locally on canvas) -->
                 <div class="wing-half top-half">
                     <div class="scaled-content">
                         <!-- Top Left Text -->
                         <div style="position: absolute; top: 12px; left: 10px; font-size: 16px; font-weight: 900; line-height: 1.15; text-align: left; color: #000; letter-spacing: 0.5px;">
                             <span style="font-family: 'Times New Roman', Times, serif;">GOLD<br/>MASTER<br/></span>
                             <span style="font-family: Arial, sans-serif; font-size: 20px;">EGYPT</span>
                         </div>
                         <!-- Top Right QR -->
                         <img src="${qrData}" style="position: absolute; top: 10px; right: 10px; width: 1.6cm; height: 1.6cm; object-fit: contain;" />
                         
                         <!-- Bottom Left Text -->
                         <div style="position: absolute; bottom: 10px; left: 10px; font-size: 11px; font-weight: 900; font-family: Arial, sans-serif; text-align: left; line-height: 1.2; color: #000;">
                             FOR CONTACT<br/><span style="font-size: 14px;">${storePhone}</span>
                         </div>
                         <!-- Bottom Right Barcode -->
                         <div style="position: absolute; bottom: 10px; right: 10px; font-size: 16px; font-weight: bold; font-family: 'Arial', sans-serif; letter-spacing: 1px; color: #000;">
                             ${data.barcode}
                         </div>
                     </div>
                 </div>
                 ...
                 <div class="wing-half bottom-half">
                     <div class="scaled-content flex-center">
                         ${store.logoBase64
                            ? '<img src="' + store.logoBase64 + '" style="max-height: 1.5cm; max-width: 90%; object-fit: contain; margin-bottom: 6px;" />'
                            : '<div style="height: 1.5cm;"></div>'
                         }
                         <div style="font-size: 24px; font-weight: 900; font-family: 'Arial', sans-serif; color: #000; margin-top: 2px;">${storeName}</div>
                     </div>
                 </div>
             `;
         };
       ...
       <html>
         <head>
           ...
           <style>
             ...
             /* Sharp scaled high-res barcode canvas style */
             .barcode-canvas {
                 max-width: 92%;
                 max-height: 0.32cm;
                 object-fit: fill;
